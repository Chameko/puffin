...
use crate::{
    ...
    id::{ExprID, StmtID},
    item_tree::SplitItemTreeNode,
    model::{Expr, Function, InternDatabase, InternStorage, Stmt},
    resolver::{ResolveDatabase, ResolveStorage},
};
use puffin_parser::{
    lexer::LexerStorage,
    parser::{ParserDatabase, ParserStorage},
};
use puffin_source::{Source, SourceDatabase, SourceStorage, SourceTree};
use puffin_vfs::AbsPathBuf;

use super::{Resolved, Upcast};
...
impl Upcast<dyn DefDatabase> for HirTest {
...
fn output_stmt(stmt: StmtID, res: &Resolved, src: &str, mut indent: usize) -> String {
    ...
    let mut output = format!(
        "{}[[ {} ]]\n",
        " ".repeat(2 * indent),
        &src[*slice.start() as usize..=*slice.end() as usize].trim()
    );
    indent += 1;
    match &res.resolved_body.stmt_alloc[stmt] {
        Stmt::ExprStmt(e) => output_expr(*e, res, src, indent),
        Stmt::Let { bind, expr } => {
            let pat_name = match &res.resolved_body.pat_alloc[bind.pat] {
                crate::model::Pattern::Ident { ident, .. } => ident.name.to_string(),
                _ => String::new(),
            };
            ...
            output.push_str(&format!(
                "{} => {:?}",
                pat_name,
                res.resolved_body.type_alloc[bind.ty].display(&res.resolved_body.type_alloc)
            ));
            if let Some(expr) = expr {
                output.push_str(&format!(
                    " from {{\n{}{}}}\n",
                    output_expr(*expr, res, src, indent),
                    " ".repeat(2 * indent)
                ))
            }
            output
